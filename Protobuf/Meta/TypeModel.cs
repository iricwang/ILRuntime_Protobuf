...
namespace ProtoBuf.Meta
{
    ...
    public abstract class TypeModel
    {
        ...
        private WireType GetWireType(ProtoTypeCode code, byte format, ref Type type, out int modelKey)
        {
            modelKey = -1;
        ...
        ///  
        /// </summary>
        internal bool TrySerializeAuxiliaryType(ProtoWriter writer,  Type type, byte format, int tag, object value, bool isInsideList)
        {
		...
		private void SerializeCore(ProtoWriter writer, Type valType, object value)
		{
			if (value == null) throw new ArgumentNullException("value");
			int key = GetKey(ref valType);
			if (key >= 0)
			{
				Serialize(key, value, writer);
			}
			else if (!TrySerializeAuxiliaryType(writer, valType, DataFormat.Default, Serializer.ListItemTag, value, false))
			{
				ThrowUnexpectedType(valType);
			}
		}
		...
		public void Serialize(Stream dest, Type valType, object value)
		{
			Serialize(dest, valType, value, null);
		}
		...
		public void Serialize(Stream dest,  Type valType, object value, SerializationContext context)
		{
			using (ProtoWriter writer = new ProtoWriter(dest, this, context))
			{
				writer.SetRootObject(value);
				SerializeCore(writer, valType, value);
				writer.Close();
			}
		}
...
//                TestEnumerableListPatterns(model, candidates, listType);
                ...
                var paramType = ProtobufTypeHelper.GetListItemType(listType);
                if (paramType != null && !candidates.Contains(paramType))
                {
...
#if !FEAT_IKVM
		private bool TryDeserializeList(TypeModel model, ProtoReader reader, byte format, int tag, Type listType, Type itemType, ref object value)
        {
            bool isList;
        ...
        /// </summary>
        internal bool TryDeserializeAuxiliaryType(ProtoReader reader, byte format, int tag, Type type, ref object value, bool skipOtherFields, bool asListItem, bool autoCreate, bool insideList)
        {
        ...
        protected internal static void ThrowUnexpectedSubtype(Type expected, Type actual)
        {
            ...
            {
                throw new InvalidOperationException(expected.FullName + " Unexpected sub-type: " + actual.FullName);
            }
        }
